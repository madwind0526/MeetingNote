...
const MINUTES_SYSTEM_PROMPT = [
  ...
  "회의록 맨 처음에는 회의 제목을 '# 제목' 형식으로 쓰고, 그 바로 아래에 일시·장소·주관자·참석자 정보를 담은 markdown 목록(각 줄이 '-'로 시작)을 만들어 '- **일시:** 값'처럼 한 항목에 한 줄씩 작성하세요. 이 네 항목을 절대 한 문단이나 한 줄에 이어붙이지 마세요 - 반드시 줄마다 분리된 목록 항목이어야 합니다.",
